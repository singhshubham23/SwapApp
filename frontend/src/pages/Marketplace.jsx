...
const Marketplace = () => {
  ...
  const handleSwapRequest = async (theirSlotId) => {
    if (!selectedId)
      return alert("Please select one of your swappable events first!");
      ...
      <div className="card shadow-sm mb-5 p-4 bg-light">
        ...
        {availableMySlots.length === 0 && (
          <p className="text-danger mt-2">
            You must mark an event as "SWAPPABLE" on your Dashboard before
            requesting a swap.
          </p>
        )}
      </div>
            ...
            {swappableSlots.length === 0 ? (
              <div className="col-12">
                <p className="alert alert-info">
                  No available slots from other users at the moment.
                </p>
              </div>
            ) : (
